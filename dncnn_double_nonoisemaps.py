...
class DnCNN(nn.Module):
    def __init__(self, in_nc=6, out_nc=3, nc=64, nb=20, act_mode='BR'):
        ...
        head1 = conv(3, nc, mode='C'+act_mode[-1], bias=bias)
        tail1 = conv(nc, out_nc, mode='C', bias=bias)
        head2 = conv(6, nc, mode='C'+act_mode[-1], bias=bias)
    ...
    def forward(self, x, train_mode=True):
        level1_out = self.model1(x)
        stage2_in = torch.cat([level1_out, x], 1)
...
class fixed_loss(nn.Module):
    ...
    def forward(self, stage1_img, stage2_img,  gt_image):
        ...
        # loss = 2 * l2_loss_stage2 + (0.5 * l2_loss_stage1) + (0.5 * asym_loss) + (0.05 * tvloss)
        loss = 2 * l2_loss_stage2 + (0.5 * l2_loss_stage1)
        return loss
